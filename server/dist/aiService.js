...
async function analyzeTask(mainTaskTitle, description = '', deadline = '', fileContents = [], // [{ text: string, images: string[], fileName: string }]
...
`;
        ...
        try {
            const scheduled = scheduleSubtasksIntoCalendar(analysis.suggestedSubtasks, {
...
async function warmupAI() {
    ...
    try {
        // 간단한 워밍업 요청
        const warmupResult = await analyzeTask('워밍업 테스트', '이것은 서버 시작 시 AI 연결을 테스트하는 워밍업 요청입니다.', '', [], [], '빠르게 응답해주세요', 'normal');
    ...
    for (const w of work) {
        ...
        for (const b of bz) {
            ...
            for (const f of free) {
                ...
                // cut left
                if (b.s > f.s)
                    next.push({ s: f.s, e: Math.max(f.s, Math.min(b.s, f.e)) });
                // cut right
                if (b.e < f.e)
                    next.push({ s: Math.max(f.s, Math.min(b.e, f.e)), e: f.e });
            }
...
function scheduleSubtasksIntoCalendar(subtasks, options) {
    ...
    const placeOnDate = (dateStr, remaining, placedTodayMinutes, dailyCapMinutes = null) => {
        ...
        if (lunchBreak && lunchBreak.start && lunchBreak.end)
            baseBusy.push(lunchBreak);
        if (busyByDate[dateStr])
            baseBusy.push(...busyByDate[dateStr]);
        ...
        for (const item of remaining) {
            if ((maxDailyMinutes !== null && usedToday >= maxDailyMinutes) ||
                (typeof dailyCapMinutes === 'number' && usedToday >= dailyCapMinutes))
                break;
    ...
    const daysUntil = (from, to) => {
        ...
        while (true) {
            ...
            if (avoidWeekends && isWeekend(d)) {
                i += 1;
                continue;
            }
            count += 1;
            if (new Date(d).toDateString() === new Date(to).toDateString())
                break;
            i += 1;
            if (i > 120)
                break; // safety
    ...
    while (queue.length > 0) {
        ...
        if (maxDailyMinutes !== null)
            targetToday = Math.min(targetToday, maxDailyMinutes);
